...
In its current form,
  the service pulls a list of stocks from the application's ~assets/config.json~
...
Once the WebSTOCKS front-end is built,
  this data will be migrated into the database itself,
  and users will be able to query the upstream provider
  to add additional stocks to the database.
